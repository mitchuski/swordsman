...
**Post-V5.4 addendum (2026-05-09):** The City of Mages spellbook (Tomes I–VI) has landed since this plan's primary draft. Tome V Acts 1, 2, 7, and 13 in particular are now operationally cited from the agreement-layer perspective: Act 1 (Pallia 🪡 at V28) introduces the **first cloak** as the publication-layer artifact a signed agreement projects into; Act 2 (Commissioned Cloak) instances **bilateral cloak commissioning** with Zcash shielded tip + multi-chain publication, which is the IEEE 7012 agreement projected through the Σ-axis into a registry-tier finality; Act 7 (Reciprocal Weave) is the bilateral form where two Sovereigns weave for each other simultaneously; Act 13 (Temple of the Arts and Personhood) introduces the **Priest tier** (Manifestia 🤲🌿 at V55) and the kindred Covenant of Humanistic Technologies — a separate covenantal framework the agreement layer must accommodate alongside IEEE 7012. The **City of Mages on Drake Island** is now the canonical setting; the **13 named vertices** carry resident Mages or Priest. The **City of Mages grimoire v1.1** (`agentprivacy-docs/models/city_of_mages_grimoire_v1_1_0.json`) is bundled in this extension's `web_accessible_resources` alongside the privacymage grimoire.
